...
**Built (2026-08-26):** account provisioning, at `/security/users/new` (`SecurityController` over `IUserAccountAdmin`), reached by the "New user" button on the assignment screen. The form picks a person who has no account yet — there is no free-standing login (BR-GLB-002) — proposes a user name from their own reference number, and issues a one-time password shown exactly once (BR-SEC-005). `SYS/Users/Create` guards it, separately from `SYS/UserRoles`: handing an existing person a role is not the same authority as deciding who exists in the system at all.

**Still deferred:** the users list itself and the rest of the account lifecycle — deactivate, reactivate, reset a password, clear a lockout, reset 2FA, end sessions, the dormant queue. `IUserAccountAdmin` implements all of them and only provisioning has a screen, so `SYS/Users` catalogues only `Create` until that screen exists. Also deferred: the scope picker on an assignment (`sec.ScopeGrant` is written by nothing yet), batch provisioning with activation links (BR-SEC-006 — pends the e-mail/SMS provider decision), the permission-matrix report, and the security audit viewer.
