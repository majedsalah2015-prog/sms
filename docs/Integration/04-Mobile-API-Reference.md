...
**74 دالة موزّعة على سبع مجموعات**، كما هبطت في اللقطة `8780f0c`. كل الصلاحيات موجودة أصلًا في
`ScreenCatalog`، فلا حاجة لإعادة تشغيل `tools/Sms.Seeder` قبل استخدام الواجهة.
...
قائمة رموز الرفض كاملة في [القسم ٩](#٩-رموز-الرفض-error-codes)، وقيم النصوص المقبولة
(الحالات، الجنس، طرق الدفع…) في [القسم ١٠](#١٠-القيم-النصية-المقبولة-enums).
...
## ١٠. القيم النصية المقبولة (Enums)
...
## ١١. ملخّص سريع — كل الدوال في جدول واحد
...
## ١٢. وثيقة OpenAPI الحيّة

على جهاز التطوير وحده: **`/api/docs`** — تعرض نفس الـ 74 دالة بحقولها ورموز رفضها، مولَّدة من
