...
## الخطوة 8 — P1: هيكل الدمج (يمكن أن يجري بالتوازي مع تطوير المدرسة)

1. `git submodule add <رابط erp-2028> external/erp` في مستودع المدرسة، مثبَّتاً على commit الخطوة 4.
2. مجلد حلّ `ERP` في `Sms.sln` يضمّ الـ13 مشروعاً المرجعية.
3. مشروع `src/Sms.Erp.Bridge` فارغ يُترجم، يعتمد على `Accounting.Contracts` **فقط**.
4. `external/erp` للقراءة فقط + فحص في خط البناء يرفض أي فرق فيه.

**بوابة القبول:** `dotnet build Sms.sln` أخضر مع 13 مشروع ERP مرجعياً، والاختبارات 1236 ما زالت خضراء.
...
```
0. remote + push للمدرسة      🔴 الأخطر
1. إنهاء Sms.Web.exe (PID 4620)
2. حذف الخردة + إيداع 61+9 ملفاً
3. نقل الوثائق إلى sms/docs
4. إنقاذ POS + الرابط + الترجمة إلى ERP_2028
5. global.json في ERP_2028
6. حذف النسخة المكرّرة        ⚠️ آخر خطوة تدميرية
7. تثبيت البنية
──────── من هنا التطوير المستقل آمن ────────
8. P1: submodule + الجسر
```
