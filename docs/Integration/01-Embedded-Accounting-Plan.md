...
### 7.1 توسيع المجموعة المستضافة — المخازن والمبيعات والمشتريات والنقدية ونقاط البيع (2026-08-22)

طلب المالك أن تكون **كل روابط ERP 2028 ضمن القسم المحاسبي**، وتحديداً: المخازن، المبيعات، المشتريات، المصروفات، القبض والصرف، ونقاط البيع. ما نُفِّذ:

| البند | القرار |
|---|---|
| **الوحدات المضافة** | `Inventory` · `Purchasing` · `Sales` (ومعها منطقة `POS`) · `Cash` · `Partners` — بخمس طبقات لكل وحدة، أي 25 مشروعاً فوق الـ15 القائمة |
| **لماذا `Partners`** | ليست طلباً مدرسياً بل **تبعية تشغيلية**: `Cash.Application/Vouchers/VoucherDrafts.cs` يحقن `Partners.Contracts.Directory`. تسجيل `Cash` بدونها يعطي حاوية تُحَلّ حتى أول مسوّدة سند |
| **"المصروفات"** | ليست وحدة قائمة بذاتها في الـ ERP: المصروف يُسجَّل بـ **سند صرف** (`Cash/PaymentVouchers`) أو **فاتورة مورّد** (`Purchasing/VendorBills`)، ويستقر على حسابات المصروفات في الدليل. لا شاشة تُسمّى «المصروفات» ولم تُخترَع واحدة |
| **`IFileStore`** | الحاجة الرابعة الوحيدة الجديدة: `Cash` يرفق صورة الشيك، و`Inventory` يحفظ أصول قوالب الملصقات. لا مُهايئ في الجسر — الـ ERP يشحن `LocalFileStore` وقسم `FileStore` في `appsettings.json` يوجّهه |
| **الهجرات** | 50 هجرة جديدة على مخططات `inv` · `pur` · `sal` · `cash` · `ptn`، بالترتيب نفسه الذي يهاجر به مضيف الـ ERP. لا تصادم مع مخططات المدرسة (§13) |
| **الصلاحيات** | 158 صلاحية في `sec.Permission` تحت `ModuleCode='ERP'`، جميعها ممنوحة لـ`SYSADMIN` — مقابل 28 في P3 |
| **الأصول الثابتة** | خمسة سكربتات ينسخها البناء بجانب `erp-theme.css`: `pos-till.js` · `erp-calculator.js` · `erp-scale-scan.js` · `erp-label-designer.js` · `erp-label-print.js` |

**التنقّل — التغيير الجوهري:** القائمة اليدوية التي شحنتها P3 (تسعة مدخلات مكتوبة بخط اليد) **حُذفت**. بدلاً منها يقرأ `Sms.Web/Navigation/ErpNavigationSource` مزوّدي `INavigationProvider` الخاصين بالـ ERP نفسه، ويحوّل كل قسم إلى مجموعة فرعية داخل قسم «المحاسبة». النتائج:

1. **لا مصدر حقيقة ثانٍ.** شاشة تُضاف في الـ ERP تظهر هنا مع أول تحديث للـ submodule. قائمة مكتوبة بخط اليد أمام سبع وحدات و~150 شاشة كانت ستتقادم بصمت.
2. **الترشيح بالصلاحية صار حقيقياً.** `INavigationMenu` يرشّح حسب مطالبات `erp.permission` التي يصكّها تسجيل الدخول. في P3 كانت مدخلات الـ ERP تُعرض للجميع وتُرفض عند الوصول — تسعة روابط ميتة كانت فوضى، و150 كانت قائمة تكذب على المستخدم.
3. **العناوين العربية من `SharedResource.ar.resx`** — الترجمة نفسها التي تعرضها شاشات الـ ERP، لا نسخة ثانية تُكتب هنا.

**ثلاث مصائد ظهرت أثناء التنفيذ:**

- **مفتاح `accounting` مكرّر:** قسم المدرسة والقسم المحاسبي في الـ ERP يحملان المفتاح ذاته، والشريط الجانبي يبني منه معرّف عنصر الطيّ — أي `#nav-accounting` مرتين في الصفحة. الحل: بادئة `erp-` على كل مفتاح قادم من الـ ERP.
- **مدخلان بعنوان URL صريح:** «سند مدين يدوي» و«سند دائن يدوي» شاشة واحدة تُبلَغ بـ`?side=Debit`/`?side=Credit`، وهو ما لا يعبّر عنه زوج متحكّم/إجراء. `NavItem` في المدرسة اكتسب خاصية `Url`، ولا يُبرَز أيٌّ منهما كشاشة حالية لأن بيانات التوجيه لا تحمل سلسلة استعلام.
- **تسريب المنطقة (خلل قائم منذ P3):** `Url.Action` يرث منطقة الطلب الحالي متى سكت المتصل عنها، فأثناء فتح شاشة ERP كان كل رابط مدرسي يولّد `/Accounting/Students` ويردّ 404. الآن **كل** مدخل يصرّح بمنطقته، والفراغ تصريح.

**العمق:** القسم صار ثلاثة مستويات (المحاسبة ← المخزون ← الأصناف)، فأعيدت كتابة `_Sidebar.cshtml` عبر جزئية `_SidebarNode.cshtml` تُصيّر نفسها تعاودياً — لأن عمق قائمة الـ ERP ليس ملكاً لهذا النظام، ومُصيِّراً مفرودَ المستويات كان سيُسقِط بصمت ما تجاوز آخر مستوى كُتب.
...
| **P3.1 — توسيع المجموعة (§7.1)** | `Inventory` · `Purchasing` · `Sales`/`POS` · `Cash` · `Partners` · تنقّل مقروء من مزوّدي الـ ERP · إصلاح تسريب المنطقة | 1 يوم | 50 هجرة مُطبَّقة على `inv pur sal cash ptn` · 158 صلاحية مفهرَسة وممنوحة · **1311 اختباراً أخضر** |
