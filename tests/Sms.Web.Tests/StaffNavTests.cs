...
{
    /// <summary>
    /// The staff tab bar showed all seven tabs to everyone, and the screens behind them answer 404
    /// to whoever does not hold them (BR-GLB-070). الثوابت is where that surfaced: the screen
    /// shipped after the schools were provisioned, so every database seeded before it had no
    /// <c>EMP/Reference</c> permission at all and the tab led nowhere for everyone — and after the
    /// seeder catalogues it, only SYSADMIN is topped up, so an HR officer still lands on "page not
    /// found".
    /// <para>
    ...
    public class StaffNavTests
    {
        ...
        {
            ["Employees"] = typeof(EmployeesController),
            ["Teachers"] = typeof(TeachersController),
        };

        /// <summary>
        /// BR-SEC-010. The one that was actually wrong in the product: a user who may open the
        /// employee directory but was never granted the reference lists is offered الثوابت and
        /// gets "page not found" for taking it up.
        /// </summary>
        [Fact]
        [BusinessRule("BR-SEC-010")]
        public async Task The_reference_tab_goes_for_a_user_without_that_screen()
        {
            var visible = await StaffNavCatalog.VisibleAsync(new StubPermissions(
                (ScreenCatalog.Modules.Employees, ScreenCatalog.Employees.Directory),
                (ScreenCatalog.Modules.Employees, ScreenCatalog.Employees.OrgChart)));

            Assert.DoesNotContain(visible, t => t.ScreenCode == ScreenCatalog.Employees.Reference);
            Assert.Equal(
                new[] { ScreenCatalog.Employees.Directory, ScreenCatalog.Employees.OrgChart },
        ...
        public async Task A_user_holding_none_of_the_screens_gets_no_tabs()
            => Assert.Empty(await StaffNavCatalog.VisibleAsync(new StubPermissions()));
        ...
        {
            var all = StaffNavCatalog.All.Select(t => (t.ModuleCode, t.ScreenCode)).ToArray();

            Assert.Equal(StaffNavCatalog.All, await StaffNavCatalog.VisibleAsync(new StubPermissions(all)));
        }
        ...
        {
            foreach (var tab in StaffNavCatalog.All)
            {
                var method = Controllers[tab.Controller]
