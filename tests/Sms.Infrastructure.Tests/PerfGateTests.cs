...
namespace Sms.Infrastructure.Tests
{
    ...
    public sealed class PerfGateTests : IDisposable
    {
        ...
        public async Task Db04_section6_gates_hold_on_the_seeded_tenant()
        {
            ...
            using (var db = CreateContext())
            {
                var refresher = new SnapshotRefreshService(db, _clock, new InstallmentAdmin(db, _clock, _audit, _tenant, new NotificationPublisher(db, new TestAddressBook()), new FeeAdmin(db, new NumberIssuer(db, _tenant, _tenant, _clock), _clock)));
